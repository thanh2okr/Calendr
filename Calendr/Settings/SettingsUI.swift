...
extension SettingsUI {
    ...
    func makeSection(title: String, content: NSView) -> DisposableWrapper<NSView> {
        ...
        if #available(macOS 26.0, *) {
            let container = NSView().forAutoLayout()
            container.wantsLayer = true
            container.layer?.cornerRadius = 10

            container.addSubview(content)
            content.edges(equalTo: container, margin: 10)

            let stackView = NSStackView(views: [label, container])
                .with(orientation: .vertical)
                .with(spacing: 6)

            stackView.setHuggingPriority(.required, for: .horizontal)

            let disposable = container.rx.updateLayer
                .map { NSColor.separatorColor.withAlphaComponent(0.1).effectiveCGColor }
                .bind(to: container.layer!.rx.backgroundColor)
            ...
            let stackView = NSStackView(views: [label, divider, content])
                .with(orientation: .vertical)
                .with(spacing: 6)
