...
class MainViewController: NSViewController {
    ...
    private let prevBtn = ImageButton(glassStyle: true)
    private let resetBtn = ImageButton(glassStyle: true)
    private let nextBtn = ImageButton(glassStyle: true)
    private let pinBtn = ImageButton(glassStyle: true)
    private let createBtn = ImageButton(glassStyle: true)
    private let remindersBtn = ImageButton(glassStyle: true)
    private let calendarBtn = ImageButton(glassStyle: true)
    private let settingsBtn = ImageButton(glassStyle: true)
    ...
    init(
        ...
        eventListView = EventListView(
            ...
            overdueColor: settingsViewModel.highlightedWeekdaysColor
        )
    ...
    override func viewDidLoad() {
        ...
        searchInput.rx.observe(\.isHidden)
            .bind { [mainStackView] in
                mainStackView.setCustomSpacing($0 ? 14 : mainStackSpacing, after: header.superview!)
            }
        ...
        // Liquid Glass container tint from calendar accent color (macOS 26)
    ...
    private func makeHeader() -> NSView {
        ...
        // Dato-style: bold month/year title, subtle nav buttons on the right
        titleLabel.font = .systemFont(ofSize: 16, weight: .bold)
        titleLabel.textColor = .headerTextColor

        [prevBtn, resetBtn, nextBtn].forEach { $0.size(equalTo: 24) }
        ...
        return NSStackView(views: [
            .spacer(width: 4), titleLabel, .spacer, prevBtn, resetBtn, nextBtn
        ])
        .with(spacing: 2)
    }
    ...
    private func makeToolBar() -> NSView {
        ...
        [pinBtn, createBtn, remindersBtn, calendarBtn, settingsBtn].forEach { $0.size(equalTo: 24) }
...
private enum Constants {
    ...
    enum EventListView {
        ...
        static let padding = NSEdgeInsets(
            left: MainStackView.margin - 4,
            bottom: MainStackView.margin / 2,
            right: MainStackView.margin - 4
        )
    }
