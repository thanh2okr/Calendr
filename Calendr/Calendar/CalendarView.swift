...
class CalendarView: NSView {
    ...
    private func setUpOutline() {
        guard #unavailable(macOS 26.0) else { return }  // glass window handles separation
    ...
    private func setUpGridBindings(_ weekCount: Int) {
        ...
        for day in 0..<weekCount * 7 {
            ...
            let dayOfWeekIndex = day % 7
            let highlightColor = Observable.combineLatest(
                viewModel.weekDays,
                viewModel.highlightedWeekdaysColor
            )
            .map { weekDays, color -> NSColor? in
                weekDays[safe: dayOfWeekIndex]?.isHighlighted == true ? color : nil
            }
            .distinctUntilChanged { $0?.hexString == $1?.hexString }
            .share(replay: 1)
...
private enum Colors {
    // Subtle outline — tertiaryLabel so it doesn't compete with glass popover
    static var outlineBackground: CGColor { NSColor.tertiaryLabelColor.effectiveCGColor }
}
