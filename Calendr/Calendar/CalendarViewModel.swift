...
class CalendarViewModel {
    ...
    init(
        ...
        highlightedWeekdaysColor = settings.highlightedWeekdaysColor
        ...
        focusedDateEventsObservable = Observable.combineLatest(
            cellViewModelsObservable, settings.futureEventsDays
        )
        .compactMap { cellViewModels, futureEventsDays in
            guard
            ...
            // Include events from future days only when futureEventsDays > 0
            let upcoming: [EventModel] = futureEventsDays > 0 ? cellViewModels
                .flatMap { vm -> [EventModel] in
                    guard dateProvider.calendar.isDate(vm.date, greaterThan: focused.date, granularity: .day) else {
                        return []
                    }
                    return vm.events
                } : []

            return DateEvents(date: focused.date, events: overdue + focused.events + upcoming)
        }
        .distinctUntilChanged()
        .share(replay: 1)
